...
El sistema de referencia S sitúa a la pared en su origen y se mueve con velocidad \( \mathbf{w} \) (igual a la pared). La velocidad relativa de la partícula en ese nuevo sistema es
...
Al colisionar con la pared se invierte el sentido según \ref{eq:velocidad_inversa_clasica}, entonces la velocidad en el sistema de referencia inicial \( S_0 \) es
...
Para analizar ambos casos es conveniente usar \textit{Cobwebs} \cite{Strogatz} , las cuales son gráficas que muestran hacia donde evoluciona la función en cada iteración. Las gráficas que se obtienen para el caso clásico en función de la velocidad de la pared se pueden ver en la figura \ref{fig:Cobweb_Classic_1D}
...
La solución encontrada son dos puntos fijos pero debemos quedarnos con la positiva, ya que la velocidad que obtenemos debe ser siempre positiva debido al valor absoluto. Además ese signo negativo en \ref{eq:Solucion_cobweb} implica usar el trozo de la función situada en el primer cuadrante. Podemos profundizar más y ver qué estabilidad tiene este punto
