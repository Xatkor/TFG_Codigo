...
\begin{align}
    \bar{u}_\parallel &= \dfrac{-u_\parallel + 2w_\parallel - u_\parallel(w_\parallel/c)^2}{1 - 2w_\parallel u_\parallel / c^2 + (w_\parallel/c)^2} \\[2mm]
    \bar{u}_\perp &= \dfrac{u_\perp\left(1 - (w_\parallel/c)^2\right)}{1 - 2w_\parallel u_\parallel / c^2 + (w_\parallel/c)^2}
\end{align}

\section{Trayectorias/Geometría de una partícula}

Como se ha podido ver tras cada colisión la partícula puede ver modificada su velocidad o no. La pregunta que surge es para que casos podemos decir que la velocidad va a aumentar y para cuales disminuirá. Además es interesante estudiar cuál es el límite de esta velocidad. 
...
Podemos observar como ambas rectas son paralelas y la nueva velocidad que obtiene la partícula va a depender de la dirección que tome la velocidad de la pared. Así en aquellos casos donde las paredes se muevan en el mismo sentido de la partícula desacelerarán la partícula hasta llegar a una velocidad \( u_k \) donde la partícula dejará de colisionar con las paredes, ya que se llegará a un punto donde \( u_k < w_n \). En el caso opueto donde las paredes siempre se dirijan hacia la partícula veremos como su velocidad aumenta sin ningún límite tras cada colisión. Estaríamos entonces presenciando la aceleración de Fermi.
...
De la solución encontrada debemos quedarnos con la positiva, ya que la velocidad que obtenemos debe ser siempre positiva debido al valor absoluto. Además ese valor negativo en \ref{eq:Solucion_cobweb} implica usar el trozo de la función situada en el primer cuadrante.

\vspace{3mm}

En un espacio de dos dimensiones no es posible realizar estas simplificaciones, pero podemos extrapolar estos resultados para preveer el comportamiento de la velocidad. En este espacio tenemos ahora un área que se verá modificada de acuerdo a la velocidad de las paredes, por ello podemos pensar que en los casos donde se vea disminuida veremos aparecer la aceleración de Fermi, ya que al menos uno de las paredes tendrá una velocidad con dirección opuesta a la partícula. Es posible encontrar alguna configuración donde las paredes consigan cancelar la aceleración producida por las otras, y esto se dará en aquellos casos donde el área se vea aumentada. 
