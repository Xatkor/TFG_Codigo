...
# Velocities of walls
top_wall_velocity = np.array([0, -2.0], dtype=np.float64)
bottom_wall_velocity = np.array([0, 3.0], dtype=np.float64)
left_wall_velocity = np.array([20.0, 0])
right_wall_velocity = np.array([15.0, 0])
...
df.to_csv(f"2D_C-N{nmax}-V-500.txt", sep="\t")
