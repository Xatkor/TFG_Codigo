...
# Velocities of walls
top_wall_velocity = np.array([0, -0.1], dtype=np.float64)
...
for j in range(nmax):
    ...
    vx, vy = 0.01 * np.array([np.cos(angle), np.sin(angle)])# Velocities must be same for every simulation
...
df.to_csv(f"2D_B-N{nmax}-V-001.txt", sep="\t")
