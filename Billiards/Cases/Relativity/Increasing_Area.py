...
# Velocities of walls
top_wall_velocity = np.asarray([0, 0.02], dtype=np.float64)
bottom_wall_velocity = np.asarray([0, 0.0], dtype=np.float64)
left_wall_velocity = np.asarray([0.0, 0])
right_wall_velocity = np.asarray([0.01, 0])
...
for j in range(nmax):
    ...
    vx, vy = 0.1 * (-1)**random.randint(1, 2), 0.1 * (-1)**random.randint(1, 2) # Velocities must be same for every simulation
...
#plt.axhline(top_wall_velocity[1], color='r', linestyle='--')
plt.show()